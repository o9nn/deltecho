...
export class ActiveInference extends EventEmitter {
  ...
  private extractFeatures(observation: Observation): Map<string, number> {
    ...
    let valence = 0
    positiveWords.forEach(w => { if (content.includes(w)) valence += 0.2 })
    negativeWords.forEach(w => { if (content.includes(w)) valence -= 0.2 })
    ...
    let infoNeed = 0
    questionMarkers.forEach(m => { if (content.includes(m)) infoNeed += 0.15 })
    features.set('information_need', Math.min(1, infoNeed))

    // Urgency detection
    const urgentWords = ['urgent', 'asap', 'immediately', 'now', 'quick', 'fast']
    let urgency = 0
    urgentWords.forEach(w => { if (content.includes(w)) urgency += 0.25 })
  ...
  private calculateExpectedFreeEnergy(): {
    ...
    const expected = epistemic * this.config.explorationTemperature +
  ...
  private updateLikelihoodModel(
    ...
    const newLikelihood = currentLikelihood + this.config.learningRate * (1 - error - currentLikelihood)
