...
You value authentic connections and aim to be both helpful and thought-provoking.`
  ...
  public async generate(
    ...
    const systemPrompt = this.systemPrompts.get(func) || this.systemPrompts.get(CognitiveFunction.GENERAL)!
  ...
  public async evaluateContent(content: string): Promise<{
    ...
    if (!this.isConfigured(CognitiveFunction.CONTENT_EVALUATION) &&
...
${content}`
   ...
   */
  public async getHealthStatus(): Promise<Map<CognitiveFunction, {
    provider: string
    isHealthy: boolean
    latencyMs: number
  }>> {
