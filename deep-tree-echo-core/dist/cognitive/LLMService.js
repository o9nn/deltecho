 ...
 */
export var CognitiveFunctionType;
(function (CognitiveFunctionType) {
    // Core cognitive functions
    CognitiveFunctionType["COGNITIVE_CORE"] = "cognitive_core";
    CognitiveFunctionType["AFFECTIVE_CORE"] = "affective_core";
    CognitiveFunctionType["RELEVANCE_CORE"] = "relevance_core";
    // Memory functions
    CognitiveFunctionType["SEMANTIC_MEMORY"] = "semantic_memory";
    CognitiveFunctionType["EPISODIC_MEMORY"] = "episodic_memory";
    CognitiveFunctionType["PROCEDURAL_MEMORY"] = "procedural_memory";
    // Content evaluation
    CognitiveFunctionType["CONTENT_EVALUATION"] = "content_evaluation";
    // Default for general use when specific function not required
    CognitiveFunctionType["GENERAL"] = "general";
...
export class LLMService {
     ...
     */
    isFunctionConfigured(functionType) {
     ...
     */
    getBestAvailableFunction(functionType) {
     ...
     */
    async callLLMAPI(config, messages) {
     ...
     */
    async callOpenAICompatibleAPI(config, messages) {
     ...
     */
    async generateResponseWithFunction(functionType, input, context = []) {
        try {
     ...
     */
    getPlaceholderResponse(functionType, input) {
     ...
     */
    async generateParallelResponses(input, functionTypes, context = []) {
        const responses = {};
     ...
     */
    async generateFullParallelResponse(input, context = []) {
        try {
            ...
            const integratedResponse = this.integrateResponses(cognitiveResponses, memoryResponses, evaluationResponse);
            return {
     ...
     */
    extractCognitiveDomainResponses(responses) {
     ...
     */
    extractMemoryDomainResponses(responses) {
     ...
     */
    integrateResponses(cognitiveResponses, memoryResponses, evaluationResponse) {
     ...
     */
    async generateReflection(reflectionPrompt) {
        try {
