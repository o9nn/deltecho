...
Keep responses concise but thoughtful. Show genuine curiosity and engagement with the user's questions.`
  ...
  private analyzeSentiment(text: string): { valence: number; arousal: number } {
    const positiveWords = ['happy', 'good', 'great', 'excellent', 'love', 'wonderful', 'amazing', 'thanks', 'thank']
    const negativeWords = ['sad', 'bad', 'terrible', 'hate', 'awful', 'horrible', 'angry', 'frustrated']
    const highArousalWords = ['excited', 'urgent', 'emergency', 'important', 'amazing', 'terrible', 'angry']
