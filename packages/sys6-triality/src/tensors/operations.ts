 ...
 */
export function add(a: ShapedTensor, b: ShapedTensor): ShapedTensor {
 ...
 */
export function sub(a: ShapedTensor, b: ShapedTensor): ShapedTensor {
  validateSameShape(a, b);
  const result = new Array(a.data.length);
  for (let i = 0; i < a.data.length; i++) {
    result[i] = (a.data[i] as number) - (b.data[i] as number);
 ...
 */
export function mul(a: ShapedTensor, b: ShapedTensor): ShapedTensor {
  validateSameShape(a, b);
  const result = new Array(a.data.length);
  for (let i = 0; i < a.data.length; i++) {
    result[i] = (a.data[i] as number) * (b.data[i] as number);
 ...
 */
export function matmul(a: ShapedTensor, b: ShapedTensor): ShapedTensor {
 ...
 */
export function dot(a: ShapedTensor, b: ShapedTensor): number {
 ...
 */
export function cat(tensors: ShapedTensor[], dim: number = 0): ShapedTensor {
  if (tensors.length === 0) {
 ...
 */
export function opponentProcess(edge: DyadicEdge): ShapedTensor {
 ...
 */
export function entangle(edge: DyadicEdge): ShapedTensor {
 ...
 */
export function extractThreadsFromFace(face: TriadicFace): {
 ...
 */
export function triadicIntegrate(
  thread_i: ShapedTensor,
  thread_j: ShapedTensor,
  thread_k: ShapedTensor
): ShapedTensor {
 ...
 */
export function extractVerticesFromBundle(bundle: TetradicBundle): {
 ...
 */
export function tetradicIntegrate(
  vertex1: ShapedTensor,
  vertex2: ShapedTensor,
  vertex3: ShapedTensor,
  vertex4: ShapedTensor
 ...
 * Clone a tensor
 */
export function clone(tensor: ShapedTensor): ShapedTensor {
 ...
 */
export function transpose(tensor: ShapedTensor, dim0: number, dim1: number): ShapedTensor {
 ...
 */
export function sum(tensor: ShapedTensor, dim?: number): ShapedTensor | number {
  if (dim === undefined) {
    // Sum all elements
    let total = 0;
    for (let i = 0; i < tensor.data.length; i++) {
 ...
 */
export function mean(tensor: ShapedTensor, dim?: number): ShapedTensor | number {
  if (dim === undefined) {
