...
export const delta3 = (t) => {
    ...
    const convolution = {
        ...
        phases,
        currentPhase: [t.phase % 3, Math.floor(t.phase / 3) % 3],
    };
 ...
 * | 4    | 1     | B    | 3     |
 */
export const phi = (clock, dyad, triad) => {
    // Calculate 4-step phase from absolute step
    const fourStepPhase = (((clock.absoluteStep - 1) % 4) + 1);
    ...
    const triadState = pattern.triad === 1 ? triad.stream1 : pattern.triad === 2 ? triad.stream2 : triad.stream3;
 ...
 */
export function calculateSyncEvents() {
    const events = [];
 ...
 */
export function getK9PhaseIndex(step) {
 ...
 */
export function getC8StateIndex(step) {
