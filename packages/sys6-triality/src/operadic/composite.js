 ...
 */
export function sys6Step(dyadic, triadic, pentadic) {
 ...
 */
export function sys6Cycle(initialDyadic, initialTriadic, dim) {
    const states = [];
    ...
    for (let absoluteStep = 1; absoluteStep <= 30; absoluteStep++) {
        ...
        const stage = Math.ceil(absoluteStep / 6);
        const stepInStage = (((absoluteStep - 1) % 6) + 1);
        const pentadic = {
            type: 'pentadic',
            stage,
            stepInStage,
 ...
 */
export function createInitialDyadic(input) {
    return {
        type: 'dyadic',
        phase: 0,
        poleA: input,
 ...
 */
export function createInitialTriadic(input) {
    return {
        type: 'triadic',
        phase: 0,
        stream1: input,
 ...
 */
export function extractFinalOutput(states) {
 ...
 */
export function validateOperadicComposition(states) {
    const errors = [];
    ...
    for (let i = 0; i < states.length; i++) {
        ...
        if (states[i].clock.absoluteStep !== expectedStep) {
            errors.push(`Step ${i}: expected absoluteStep ${expectedStep}, got ${states[i].clock.absoluteStep}`);
        }
    }
    // Check dyadic phase alternation (mod 2)
    for (let i = 0; i < states.length; i++) {
        const expectedPhase = ((i + 1) % 2);
        if (states[i].clock.dyadicPhase !== expectedPhase) {
            errors.push(`Step ${i + 1}: expected dyadicPhase ${expectedPhase}, got ${states[i].clock.dyadicPhase}`);
        }
    }
    // Check triadic phase rotation (mod 3)
    for (let i = 0; i < states.length; i++) {
        const expectedPhase = ((i + 1) % 3);
        if (states[i].clock.triadicPhase !== expectedPhase) {
            errors.push(`Step ${i + 1}: expected triadicPhase ${expectedPhase}, got ${states[i].clock.triadicPhase}`);
        }
    }
    // Check pentadic stage boundaries (every 6 steps)
    for (let i = 0; i < states.length; i++) {
        const expectedStage = Math.ceil((i + 1) / 6);
        if (states[i].clock.pentadicStage !== expectedStage) {
            errors.push(`Step ${i + 1}: expected pentadicStage ${expectedStage}, got ${states[i].clock.pentadicStage}`);
        }
    }
    // Check cubic concurrency has 8 states
    for (let i = 0; i < states.length; i++) {
        if (states[i].cubic.states.length !== 8) {
            errors.push(`Step ${i + 1}: cubic concurrency should have 8 states, got ${states[i].cubic.states.length}`);
        }
    }
    // Check triadic convolution has 9 phases
    for (let i = 0; i < states.length; i++) {
        if (states[i].convolution.phases.length !== 9) {
            errors.push(`Step ${i + 1}: triadic convolution should have 9 phases, got ${states[i].convolution.phases.length}`);
        }
    }
