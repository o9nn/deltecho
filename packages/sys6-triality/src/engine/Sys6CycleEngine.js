 ...
 */
import { createTensor, zeros, createTetradicBundle, createDyadicEdge, createTriadicFace, toStepAddress, getDoubleStepDelayState, getPrimaryStreamForStep, getStreamPerceptions, getDyadicPairForStep, getTriadicPermutationsForStep, } from '../tensors/index.js';
import { add, cat } from '../tensors/operations.js';
import { Module, Sequential, ModuleList } from '../modules/Module.js';
import { Linear, LayerNorm, GELU, GRUCell, LSTM } from '../modules/layers.js';
import { TriadicConvolution, TetradicConvolution } from '../modules/convolutions.js';
/**
...
export class Sys6CycleEngine extends Module {
    ...
    constructor(config) {
        ...
        for (let p = 0; p < 3; p++) {
            const phaseSteps = [];
            for (let s = 0; s < 5; s++) {
                const steps = new ModuleList([new TriadicConvolution(dim), new TriadicConvolution(dim)]);
        ...
        this.integrationLayer = new Sequential(new Linear(dim * 3, hiddenDim), new GELU(), new LayerNorm(hiddenDim), new Linear(hiddenDim, dim), new LayerNorm(dim));
        this.registerModule('integration', this.integrationLayer);
    }
    _makePhaseProcessor() {
        return new Sequential(new Linear(this.config.dim, this.config.hiddenDim), new LayerNorm(this.config.hiddenDim), new GELU(), new Linear(this.config.hiddenDim, this.config.dim));
    }
    _makeStageProcessor() {
        return new Sequential(new Linear(this.config.dim, this.config.dim), new LayerNorm(this.config.dim), new GELU());
    }
    _makeStreamProcessor() {
        return new Sequential(new Linear(this.config.dim, this.config.dim), new LayerNorm(this.config.dim), new GELU());
    }
    /**
    ...
    _initializeStreams(inputState) {
        ...
        for (let b = 0; b < batchSize; b++) {
            for (let i = 0; i < dim; i++) {
                const val = inputState.data[b * dim + i];
     ...
     */
    _initializeTetradic(inputState) {
        const { dim } = this.config;
        const batchSize = inputState.shape[0];
     ...
     */
    _executeStep(absoluteStep, currentState, streams, tetradic, telemetryHidden, dyadState, triadState) {
        const { dim } = this.config;
        ...
        const [telemetryOut, newTelemetryHidden] = this.telemetry.forward(telemetryInput, telemetryHidden);
    ...
    _stateToTriadicFace(state) {
        ...
        return createTriadicFace(createDyadicEdge(v_i, v_j, 'edge_12'), // edge_ij: vertex i to vertex j
     ...
     */
    _processStreams(streams, state, absoluteStep) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            newStreams.push({
                ...
                state: newState,
                phase,
                stage: ((Math.floor(phaseRotation / 2) % 5) + 1),
                perceives: {
     ...
     */
    _reshapeForLSTM(state) {
        const batchSize = state.shape[0];
        const dim = state.shape[1];
        // Add sequence dimension: [batch, dim] -> [batch, 1, dim]
        return createTensor(Array.from(state.data), [batchSize, 1, dim], state.dtype);
    }
    /**
     * Reshape tensor from LSTM output
     */
    _reshapeFromLSTM(output, batchSize) {
        // Remove sequence dimension: [batch, 1, hidden] -> [batch, hidden]
        const hiddenDim = output.shape[2];
        return createTensor(Array.from(output.data), [batchSize, hiddenDim], output.dtype);
    }
    /**
     * Run the complete 30-step cognitive cycle
     */
    forward(inputState, numCycles = 1) {
        ...
        for (let b = 0; b < batchSize; b++) {
            dyadState.data[b * 2] = 1;
            triadState.data[b * 3] = 1;
        }
        const allSteps = [];
        // Run cycles
        for (let cycle = 0; cycle < numCycles; cycle++) {
            ...
            for (let step = 1; step <= 30; step++) {
                const stepResult = this._executeStep(step, currentState, streams, tetradic, telemetryHidden, dyadState, triadState);
        ...
        const streamStates = cat([streams[0].state, streams[1].state, streams[2].state], -1);
     ...
     */
    step(absoluteStep, currentState, streams, tetradic, telemetryHidden, dyadState, triadState) {
        const batchSize = currentState.shape[0];
        ...
        return this._executeStep(absoluteStep, currentState, streams, tetradic, telemetryHidden, dyadState, triadState);
    }
    /**
     * Get information about a specific step
     */
    getStepInfo(absoluteStep) {
...
//# sourceMappingURL=Sys6CycleEngine.js.map