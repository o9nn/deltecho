...
export class DyadicConvolution extends Module {
  private poleATransform: Linear;
  private poleBTransform: Linear;
  private entanglementLayer: Bilinear;
  private outputNorm: LayerNorm;
   ...
   */
  forwardWithEntanglement(edge: DyadicEdge): { edge: DyadicEdge; entangled: ShapedTensor } {
    const aPrime = this.poleATransform.forward(edge.poleA);
    const bPrime = this.poleBTransform.forward(edge.poleB);
    const entangled = this.entanglementLayer.forward(aPrime, bPrime);
...
export class TriadicConvolution extends Module {
  private edgeConv_ij: DyadicConvolution;
  private edgeConv_jk: DyadicConvolution;
  private edgeConv_ki: DyadicConvolution;
  ...
  forward(face: TriadicFace): { face: TriadicFace; integrated: ShapedTensor } {
  ...
  forwardWithEntanglement(face: TriadicFace): {
    ...
    const newFace = createTriadicFace(
      result_ij.edge,
      result_jk.edge,
      result_ki.edge,
      face.faceId
...
export class TetradicConvolution extends Module {
  private faceConv_123: TriadicConvolution;
  private faceConv_124: TriadicConvolution;
  private faceConv_134: TriadicConvolution;
  private faceConv_234: TriadicConvolution;
  private vertexIntegration: Sequential;
  ...
  constructor(dim: number) {
    ...
    this.vertexIntegration = new Sequential(
      new Linear(dim * 4, dim * 3),
      new GELU(),
      new Linear(dim * 3, dim * 2),
      new GELU(),
      new Linear(dim * 2, dim),
  ...
  forward(bundle: TetradicBundle): { bundle: TetradicBundle; integrated: ShapedTensor } {
    ...
    const newBundle: TetradicBundle = {
...
export class NestedNeuralNetwork extends Module {
  private levels: Module[][];
  
  readonly dim: number;
  readonly nestingLevels: number;
  readonly termsPerLevel: number[];
  
  constructor(dim: number, nestingLevels: number = 4) {
    ...
    for (let level = 0; level < nestingLevels; level++) {
      ...
      const levelModules: Module[] = [];
      
      for (let t = 0; t < terms; t++) {
        const module = new Sequential(
          new Linear(currentDim, currentDim),
          new LayerNorm(currentDim),
          new GELU()
        );
  ...
  forwardByLevel(input: ShapedTensor): ShapedTensor[][] {
    const outputsByLevel: ShapedTensor[][] = [];
    let currentInputs = [input];
