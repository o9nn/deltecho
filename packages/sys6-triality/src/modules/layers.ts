...
export class Linear extends Module {
  ...
  forward(input: ShapedTensor): ShapedTensor {
    ...
    const weightT = createTensor(
      this._transposeWeight(),
  ...
  private _transposeWeight(): number[] {
    ...
    for (let i = 0; i < this.outFeatures; i++) {
      for (let j = 0; j < this.inFeatures; j++) {
        result[j * this.outFeatures + i] = this.weight.data.data[i * this.inFeatures + j] as number;
      }
    }
    return result;
  }
  ...
  forward(input: ShapedTensor): ShapedTensor {
    ...
    for (let b = 0; b < batchSize; b++) {
      for (let i = 0; i < normalizedSize; i++) {
        ...
        result[idx] = (normalized.data[idx] as number) * (this.weight.data.data[i] as number) 
                    + (this.bias.data.data[i] as number);
      }
    }
  ...
  forward(input: ShapedTensor): ShapedTensor {
    ...
    for (let i = 0; i < input.data.length; i++) {
      if (Math.random() < this.p) {
        ...
        result[i] = (input.data[i] as number) * scale;
      }
    }
      ...
      for (let o = 0; o < this.outFeatures; o++) {
        ...
        for (let i = 0; i < this.in1Features; i++) {
          for (let j = 0; j < this.in2Features; j++) {
            ...
            const x1 = input1.data[b * this.in1Features + i] as number;
            const x2 = input2.data[b * this.in2Features + j] as number;
            sum += (this.weight.data.data[wIdx] as number) * x1 * x2;
          }
        }
        
        if (this.bias) {
          sum += this.bias.data.data[o] as number;
        }
...
export class Embedding extends Module {
  ...
  forward(indices: ShapedTensor): ShapedTensor {
    ...
    const flatIndices = Array.from(indices.data).map(x => Math.floor(x as number));
    ...
    for (let i = 0; i < flatIndices.length; i++) {
      ...
      for (let j = 0; j < this.embeddingDim; j++) {
        result[i * this.embeddingDim + j] = this.weight.data.data[idx * this.embeddingDim + j] as number;
      }
    }
    ...
    for (let b = 0; b < batchSize; b++) {
      ...
      for (let g = 0; g < 3 * this.hiddenSize; g++) {
        ...
        for (let i = 0; i < this.inputSize; i++) {
          sum += (this.weight_ih.data.data[g * this.inputSize + i] as number) 
               * (input.data[b * this.inputSize + i] as number);
        }
        if (this.bias_ih) {
          sum += this.bias_ih.data.data[g] as number;
        }
        gates_ih[g] = sum;
      }
      ...
      for (let g = 0; g < 3 * this.hiddenSize; g++) {
        ...
        for (let h = 0; h < this.hiddenSize; h++) {
          sum += (this.weight_hh.data.data[g * this.hiddenSize + h] as number)
               * (hidden.data[b * this.hiddenSize + h] as number);
        }
        if (this.bias_hh) {
          sum += this.bias_hh.data.data[g] as number;
        }
        gates_hh[g] = sum;
      }
      ...
      for (let h = 0; h < this.hiddenSize; h++) {
        ...
        const z = 1 / (1 + Math.exp(-(gates_ih[this.hiddenSize + h] + gates_hh[this.hiddenSize + h])));
        const n = Math.tanh(gates_ih[2 * this.hiddenSize + h] + r * gates_hh[2 * this.hiddenSize + h]);
        
        const hPrev = hidden.data[b * this.hiddenSize + h] as number;
...
export class LSTM extends Module {
  ...
  forward(
    ...
    let h = hidden?.[0] || zeros([this.numLayers, batchSize, this.hiddenSize]);
    let c = hidden?.[1] || zeros([this.numLayers, batchSize, this.hiddenSize]);
    ...
    for (let t = 0; t < seqLen; t++) {
      ...
      for (let b = 0; b < batchSize; b++) {
        for (let i = 0; i < this.inputSize; i++) {
          ...
          inputT[b * this.inputSize + i] = input.data[idx] as number;
        }
      }
      ...
      for (let l = 0; l < this.numLayers; l++) {
        ...
        for (let b = 0; b < batchSize; b++) {
          for (let i = 0; i < this.hiddenSize; i++) {
            hL[b * this.hiddenSize + i] = h.data[l * batchSize * this.hiddenSize + b * this.hiddenSize + i] as number;
          }
        }
        ...
        for (let b = 0; b < batchSize; b++) {
          for (let i = 0; i < this.hiddenSize; i++) {
            ...
            (h.data as number[])[hIdx] = newHidden.data[b * this.hiddenSize + i] as number;
          }
        }
