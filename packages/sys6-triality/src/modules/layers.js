 ...
 */
import { createTensor, zeros } from '../tensors/index.js';
import { add, matmul, reshape, layerNorm as layerNormOp, gelu, relu, sigmoid, tanh as tanhOp, softmax as softmaxOp, } from '../tensors/operations.js';
import { Module, createParameter } from './Module.js';
/**
...
export class Linear extends Module {
    ...
    forward(input) {
        ...
        if (this.bias) {
            ...
            const biasExpanded = createTensor(new Array(batchSize).fill(0).flatMap(() => Array.from(this.bias.data.data)), [batchSize, this.outFeatures]);
...
export class Bilinear extends Module {
    ...
    in2Features;
    outFeatures;
    constructor(in1Features, in2Features, outFeatures, bias = true) {
        super('Bilinear');
        this.in1Features = in1Features;
        this.in2Features = in2Features;
        this.outFeatures = outFeatures;
        // Weight: [outFeatures, in1Features, in2Features]
        this.weight = createParameter('weight', [outFeatures, in1Features, in2Features], 'xavier_uniform');
        this.registerParameter('weight', this.weight);
        if (bias) {
            this.bias = createParameter('bias', [outFeatures], 'zeros');
            this.registerParameter('bias', this.bias);
...
export class LSTM extends Module {
    cells; // Simplified: using GRU cells for now
    inputSize;
    hiddenSize;
    numLayers;
    batchFirst;
    constructor(inputSize, hiddenSize, numLayers = 1, batchFirst = true) {
        super('LSTM');
        this.inputSize = inputSize;
        this.hiddenSize = hiddenSize;
            ...
            for (let l = 0; l < this.numLayers; l++) {
                ...
                for (let b = 0; b < batchSize; b++) {
                    for (let i = 0; i < this.hiddenSize; i++) {
                        ...
                        h.data[hIdx] = newHidden.data[b * this.hiddenSize + i];
                    }
                }
