...
export class AdaptivePersonality {
  ...
  constructor(
    ...
    if (initialPersonality) {
      // Filter out undefined values when merging
      const validPersonalityValues = Object.entries(initialPersonality)
        .filter(([_, value]) => value !== undefined)
        .reduce((acc, [key, value]) => ({ ...acc, [key]: value }), {}) as PersonalityVector;
  ...
  public updateEmotionalState(
    ...
    for (const [dimension, change] of Object.entries(emotionalUpdate)) {
      if (dimension in this.currentEmotionalState) {
        ...
        this.currentEmotionalState[dimension as keyof EmotionalState] = Math.max(
          0,
          Math.min(1, newValue)
        );
      }
   ...
   */
  public recordInteraction(userId: string, intensity: number, sentiment: number): void {
  ...
  public analyzePersonalityEvolution(): {
    ...
    const contextChanges = this.personalityHistory
      .slice(1)
      .filter(
        (snapshot, i) => snapshot.activeContext !== this.personalityHistory[i].activeContext
      ).length;
  ...
  private recalculateEmotionalBalance(): void {
    ...
    const extremityPenalty = Math.max(0, positive - 0.8) * 0.5 + Math.max(0, negative - 0.5) * 0.7;

    this.currentEmotionalState.balance = Math.max(0, Math.min(1, rawBalance - extremityPenalty));
  }

  /**
   * Shifts personality dimensions with constraints to maintain coherence
   */
  private shiftPersonality(shifts: Partial<PersonalityVector>, intensity: number): void {
  ...
  private evolveThroughInteraction(): void {
    ...
    const avgSentiment =
      this.recentInteractions.reduce((sum, interaction) => sum + interaction.sentiment, 0) /
      this.recentInteractions.length;

    // Calculate average intensity
    const avgIntensity =
      this.recentInteractions.reduce((sum, interaction) => sum + interaction.intensity, 0) /
      this.recentInteractions.length;
    ...
    for (const emotion of emotions) {
      ...
      for (const dimension of dimensions) {
        const correlationData = this.personalityHistory.map((snapshot) => ({
          emotionValue: snapshot.emotionalState[emotion as keyof EmotionalState],
