...
export class SecureIntegration {
  ...
  public async secureStore(
    ...
    const requiredLevel =
      options.requiredEncryptionLevel || this.getDefaultEncryptionLevel(options.dataType);
  ...
  public async secureRetrieve(key: string): Promise<any | null> {
    ...
    try {
      ...
      if (storedData) {
        ...
        const decrypted = await this.decryptData(storedData, SecureState.LOCALLY_ENCRYPTED);
        return decrypted;
      }
    } catch (err) {
  ...
  public async createSecureExport(
    ...
    for (const [key, item] of this.secureMemoryStore.entries()) {
      if (dataTypes.includes(item.type)) {
        const decrypted = await this.decryptData(item.data, item.encryptionState);
        exportData[key] = {
          data: decrypted,
          type: item.type,
   ...
   */
  private async encryptData(data: any, encryptionState: SecureState): Promise<string> {
   ...
   */
  private async decryptData(encryptedData: string, encryptionState: SecureState): Promise<any> {
    switch (encryptionState) {
      case SecureState.VERIFIED_ENCRYPTED:
      case SecureState.END_TO_END_ENCRYPTED:
        // In real implementation, would use DeltaChat's E2E decryption
        // For demo, just use a simple decryption
        return this.simpleDecrypt(encryptedData, this.localEncryptionKey + '_e2e');
   ...
   */
  private async encryptWithCustomKey(data: string, key: string): Promise<string> {
   ...
   */
  private async decryptWithCustomKey(data: string, key: string): Promise<string> {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (storageKey?.startsWith('secure_cognitive_')) {
        try {
          ...
          console.error(`Failed to parse secure storage data for index ${i}:`, err);
        }
      }
    }
