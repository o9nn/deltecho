...
export class EmotionalIntelligence {
  ...
  private updateEmotionalState(
    ...
    for (const [emotion, intensity] of Object.entries(newEmotions)) {
      const currentIntensity =
        this.currentEmotionProfile.primaryEmotions[emotion as EmotionDimension] || 0;

      // Weighted blend based on sensitivity
      this.currentEmotionProfile.primaryEmotions[emotion as EmotionDimension] =
        currentIntensity * (1 - this.sensitivityLevel) + intensity * this.sensitivityLevel;
    }
  ...
  private calculateValenceArousal(profile: EmotionProfile): void {
    ...
    const totalIntensity =
      positiveValence + negativeValence + (primary[EmotionDimension.SURPRISE] || 0);

    // Calculate normalized valence (-1 to 1)
    profile.valence = totalIntensity > 0 ? (positiveValence - negativeValence) / totalIntensity : 0;
    ...
    const lowArousalContribution =
      (primary[EmotionDimension.SADNESS] || 0) * 0.3 + (primary[EmotionDimension.TRUST] || 0) * 0.2;
  ...
  private calculateDominantEmotions(): [EmotionDimension, number][] {
    ...
    for (const entry of this.emotionHistory) {
      const dominantEmotion = entry.profile.dominantEmotion;
      if (dominantEmotion) {
        emotionCounts.set(dominantEmotion, (emotionCounts.get(dominantEmotion) || 0) + 1);
      }
    }

    // Convert to array and calculate percentages
    const total = this.emotionHistory.length;
    const entries = Array.from(emotionCounts.entries());
    const dominantEmotions: [EmotionDimension, number][] = entries
  ...
  private generateEmotionalPhrases(profile: EmotionProfile): string[] {
    ...
    // Default phrases
    const phrases: string[] = ['I understand', 'That makes sense', 'I see what you mean'];

    // Add emotion-specific phrases
    if (dominantEmotion) {
      switch (dominantEmotion) {
        case EmotionDimension.JOY:
          phrases.push("That's wonderful!", "I'm happy to hear that", 'That sounds delightful');
          break;
        case EmotionDimension.SADNESS:
          phrases.push(
