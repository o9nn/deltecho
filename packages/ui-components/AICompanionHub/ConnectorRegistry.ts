...
import { CharacterAIConnector, CharacterAIConfig } from './connectors/CharacterAIConnector.js';
...
export class ConnectorRegistry extends EventEmitter {
   ...
   */
  public startNewConversation(connectorId: string, initialMessage?: string): string {
    const conversationId = `conv_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;

    if (initialMessage) {
      this.sendMessage(connectorId, conversationId, initialMessage).catch((error) => {
   ...
   */
  public async searchMemories(query: string, connectorId?: string): Promise<AIMemory[]> {
   ...
   */
  public async findRelatedMemories(memoryId: string, limit?: number): Promise<AIMemory[]> {
  ...
  private async saveConnectorConfigs(): Promise<void> {
    try {
      ...
      await runtime.setDesktopSetting(
        'aiConnectors' as any,
        JSON.stringify(connectorConfigs) as any
      );
    } catch (error) {
