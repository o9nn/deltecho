...
export class MemoryPersistenceLayer extends EventEmitter {
  ...
  private async consolidateMemories(): Promise<void> {
    ...
    this.companions.forEach((companionId) => {
      companionMemories[companionId] = Array.from(this.memories.values()).filter(
        (memory) => memory.companionId === companionId
      );
    });
    ...
    for (const companionId of this.companions) {
      ...
      for (const memory of memories) {
        ...
        relatedMemoryIds.forEach((relatedId) => {
          ...
          const timeProximity =
            1 / (1 + Math.abs(memory.timestamp - relatedMemory.timestamp) / (24 * 60 * 60 * 1000));

          // Calculate relationship strength
          const strength =
            (0.7 * sharedTopics) / Math.max(memory.topics.length, relatedMemory.topics.length) +
            0.3 * timeProximity;
