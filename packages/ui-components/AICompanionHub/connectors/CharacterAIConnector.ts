  ...
  async authenticate(): Promise<boolean> {
    try {
      ...
      if (!authResponse.ok) {
        ...
        throw new Error(
          `Character.AI authentication failed: ${errorData.error || authResponse.statusText}`
        );
      }
  ...
  private async loadCharacterDefinition(characterId: string): Promise<any> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to load character definition: ${errorData.error || response.statusText}`
        );
      }
  ...
  async getAvailableCharacters(): Promise<any[]> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          `Failed to load available characters: ${errorData.error || response.statusText}`
        );
      }
  ...
  async searchCharacters(query: string): Promise<any[]> {
    try {
      ...
      const response = await fetch(
        `https://beta.character.ai/chat/characters/search/?query=${encodeURIComponent(query)}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Token ${this.authToken}`,
