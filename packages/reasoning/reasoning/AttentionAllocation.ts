...
export class AttentionAllocation {
  ...
  stimulate(atomId: string, amount: number): void {
    ...
    const newSTI = Math.min(this.config.maxSTI, atom.attentionValue.sti + amount);
  ...
  applyHebbianLearning(atom1Id: string, atom2Id: string): void {
    ...
    let link = links.find((l) => l.outgoing?.includes(atom1Id) && l.outgoing?.includes(atom2Id));
    ...
    // Strengthen the link
    const newStrength = Math.min(1.0, link.truthValue.strength + this.config.hebbianLearningRate);
  ...
  collectRent(stiThreshold: number = -100, ltiThreshold: number = 0): number {
    ...
    for (const atom of atoms) {
      if (atom.attentionValue.sti < stiThreshold && atom.attentionValue.lti < ltiThreshold) {
