...
export class TrajectoryDistribution {
   ...
   */
  private partitionNode(echo: DeepTreeEcho, node: DistributionNode, depth: number): void {
    ...
    const partitionNum =
      this.config.partitionSequence[depth % this.config.partitionSequence.length];
   ...
   */
  private clusterTrajectories(trajectories: Trajectory[], k: number): Trajectory[][] {
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      ...
      const newAssignments = features.map((f) => this.findNearestCentroid(f, centroids));
  ...
  private extractFeatures(trajectory: Trajectory): number[] {
    ...
    const avgVelocity =
      trajectory.actions.reduce((sum, a) => sum + (a.velocity || 0), 0) /
  ...
  private calculateStats(trajectories: Trajectory[]): {
    ...
    const avgDuration =
      trajectories.reduce((sum, t) => sum + (t.endTime - t.startTime), 0) / trajectories.length;
