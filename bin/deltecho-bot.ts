...
const tools: Anthropic.Tool[] = [
  {
    name: 'bash',
    description: 'Execute bash commands. Use for file operations, git, builds, etc.',
    input_schema: {
...
async function main() {
  ...
  if (firstAccount.kind === 'Unconfigured') {
    ...
    try {
      if (process.env.ADDR && process.env.MAIL_PW) {
        ...
        throw new Error('Credentials missing: Set ADDR and MAIL_PW, or use CHATMAIL_QR');
      }
      await dc.rpc.batchSetConfig(firstAccount.id, {
        bot: '1',
        e2ee_enabled: '1',
  ...
  const verificationQRCode = (await dc.rpc.getChatSecurejoinQrCodeSvg(botAccountId, null))[0];
