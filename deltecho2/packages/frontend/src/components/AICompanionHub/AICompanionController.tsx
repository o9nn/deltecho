...
const log = getLogger('render/components/AICompanionHub/AICompanionController')
...
export const AICompanionProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    const initializeAICompanions = async () => {
      try {
        ...
        log.error('Failed to initialize AI Companion system:', error)
    ...
    return () => {
      ...
      if (registry) {
        registry
          .shutdown()
          .catch(error => log.error('Failed to shut down registry:', error))
      }
    }
  }, [])
  ...
  const setActiveCompanion = async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      log.error(`Failed to set active companion ${id}:`, error)
      setError(error instanceof Error ? error.message : String(error))
      return false
    }
  }
  ...
  const getRelatedMemories = async (memoryId: string): Promise<AIMemory[]> => {
    ...
    try {
      ...
      log.error(`Failed to get related memories for ${memoryId}:`, error)
      setError(error instanceof Error ? error.message : String(error))
      throw error
    }
  }
  ...
  const authenticateCompanion = async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      log.error(`Failed to authenticate companion ${id}:`, error)
      setError(error instanceof Error ? error.message : String(error))
      throw error
    }
  }
  ...
  const updateCompanionConfig = async (
    ...
    try {
      ...
      log.error(`Failed to update companion ${id}:`, error)
      setError(error instanceof Error ? error.message : String(error))
      throw error
    }
  }
  ...
  const createCompanion = async (config: any): Promise<string> => {
    ...
    try {
      ...
      log.error('Failed to create companion:', error)
      setError(error instanceof Error ? error.message : String(error))
      throw error
    }
  }
  ...
  const removeCompanion = async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      log.error(`Failed to remove companion ${id}:`, error)
      setError(error instanceof Error ? error.message : String(error))
      throw error
    }
  }
