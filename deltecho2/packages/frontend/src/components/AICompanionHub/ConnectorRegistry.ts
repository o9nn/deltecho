...
const log = getLogger('render/components/AICompanionHub/ConnectorRegistry')
...
export class ConnectorRegistry extends EventEmitter {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      log.error('Failed to initialize AI Connector Registry:', error)
      throw error
    }
  }
  ...
  public async authenticateConnector(id: string): Promise<boolean> {
    ...
    try {
      ...
      log.error(`Error authenticating connector ${id}:`, error)
      this.activeConnectors.delete(id)
  ...
  public async sendMessage(
    ...
    try {
      ...
      log.error(`Error sending message to connector ${connectorId}:`, error)
  ...
  private async saveConnectorConfigs(): Promise<void> {
    try {
      ...
      log.error('Failed to save connector configurations:', error)
      throw error
    }
  }
    ...
    connector.on('configUpdated', config => {
      ...
      if (id) {
        ...
        this.saveConnectorConfigs().catch(error => {
          log.error('Failed to save connector configuration update:', error)
        })
  ...
  public async shutdown(): Promise<void> {
    ...
    log.info('AI Connector Registry shut down')
  }
}
