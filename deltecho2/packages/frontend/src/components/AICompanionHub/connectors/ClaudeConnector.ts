...
const log = getLogger(
  'render/components/AICompanionHub/connectors/ClaudeConnector'
)
...
export class ClaudeConnector extends BaseConnector {
  ...
  async authenticate(): Promise<boolean> {
    try {
      ...
      log.error('Claude authentication error:', error)
    ...
    try {
      ...
      if (content.includes('```json') && functions && functions.length > 0) {
        try {
          ...
          log.warn('Failed to parse potential function call:', e)
        }
      }
      ...
      log.error('Claude response generation error:', error)
      throw error
    }
  }
