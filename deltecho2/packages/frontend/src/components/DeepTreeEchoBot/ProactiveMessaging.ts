...
export class ProactiveMessaging {
  ...
  private async processQueue(): Promise<void> {
    ...
    for (const msg of readyMessages) {
      ...
      // Trust gate: only send proactive messages to trusted contacts
      if (!relationshipTrustGate.isProactiveEligible(msg.chatId)) {
        msg.status = 'failed'
        msg.error = 'Contact not trusted enough for proactive messaging'
        log().info(`Skipped message ${msg.id}: contact not proactive-eligible`)
        continue
      }
