...
describe('Integration', () => {
  it('should export all required singletons', () => {
    // Note: asserted on the individual modules rather than the index barrel,
    // because deltecho2's barrel pulls in UI modules that need broader mocks.
    const {
      chatManager,
      getChatManager,
    } = require('../DeepTreeEchoChatManager')
    const { uiBridge, getUIBridge } = require('../DeepTreeEchoUIBridge')
    const {
      proactiveMessaging,
    ...
    expect(getChatManager).toBeDefined()
    expect(uiBridge).toBeDefined()
    expect(getUIBridge).toBeDefined()
    expect(proactiveMessaging).toBeDefined()
    expect(getProactiveMessaging).toBeDefined()
  })
})
