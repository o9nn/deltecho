...
type MaybeProcessEnv = {
  process?: { env?: Record<string, string | undefined> }
}

function readEnv(name: string): string | undefined {
  // Browser-safe access: `process` only exists when a bundler injects it
  // or when running under Node (e.g. Jest).
...
export function getDefaultLLMEndpoint(): string {
  return readEnv('VITE_APHRODITECHO_LLM_URL') ?? PUBLIC_OPENAI_ENDPOINT
}

export function getDefaultLLMModel(): string {
  return readEnv('VITE_APHRODITECHO_MODEL') ?? 'gpt-4'
}
