...
1. **Kernel Function Implementation**
   - Map KoboldAI modules → Kernel primitives:
     - Story Management → Memory Tensor Allocator (`story_alloc`, `chunk_tensor`)
     - Context Assembly → Dynamic Context Builder (`ctx_assemble_tensor`, `worldinfo_inject`)
     - Generation Pipeline → Sampler Control (`sample_nucleus`, `sample_topk_tensor`)
     - World Building → Graph Structure Allocator (`world_graph_init`, `location_tensor`)
     - Agent Orchestration → Multi-Agent Scheduler (`agent_sched_tick`, `collab_matrix`)
...
- **Performance:** ≤1ms context assembly, ≤500µs token sampling, ≤2ms world info scan
